...
/*      AXI4 DMAC
  ...
  0x10: cfgReg
 */
class AXI4DMAC(address: Seq[AddressSet])(implicit p: Parameters)extends AXI4SlaveModule(address, executable = false){
  ...
  override lazy val module = new AXI4SlaveModuleImp[Null](this) {
    ...
    //dontTouch(masterNode)
    dontTouch(masterNode.out.head._1)
